...
impl incoming_handler::Handler<Option<Context>> for CustomTemplateProvider {
    async fn handle(
        &self,
        ctx: Option<Context>,
        _request: ResourceOwn<IncomingRequest>,
        response_out: ResourceOwn<ResponseOutparam>,
        ...
        info!(
            ...
            "received request for incoming_handler"
        );

        let client =
            wasmcloud_provider_sdk::get_connection().get_wrpc_client(&requesting_component);

        info!("creating response body");

        // Build response body
        let response =
            OutgoingResponse::new(&client, None, &Fields::new(&client, None).await.unwrap())
                .await
                .unwrap();

        info!("setting status code");

        OutgoingResponse::set_status_code(&client, None, &response.as_borrow(), 200)
            .await
            .unwrap()
            .unwrap();
        ...
        let response_body = OutgoingResponse::body(&client, None, &response.as_borrow())
            .await
            .unwrap()
            .unwrap();

        info!("setting response");

        ResponseOutparam::set(&client, None, &response_out, &Ok(response))
            .await
            .unwrap();

        info!("creating output stream");

        let output_stream = OutgoingBody::write(&client, None, &response_body.as_borrow())
            .await
            .unwrap()
            .unwrap();
        ...
        OutputStream::blocking_write_and_flush(
            &client,
            None,
            &output_stream.as_borrow(),
        ...
        OutgoingBody::finish(&client, None, &response_body, None)
            .await
            .unwrap()
            .unwrap();
